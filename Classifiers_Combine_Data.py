...
def load_dataset_group(data_type, group, prefix, filenames):
    if data_type == 'combined':
        ...
        for name in filenames:
            data = load_file(prefix + 'Processed_PatientSpace' + '/' + group + name + '_' + 'Processed_PatientSpace' + '.csv')
            a.extend(data)
        df_a = pd.DataFrame(a)
        b = []
        # load input data
        for name in filenames:
            data = load_file(prefix + 'Processed_RawXY' + '/' + group + name + '_' + 'Processed_RawXY' + '.csv')
            b.extend(data)
        df_b = pd.DataFrame(b)
        df_x = pd.concat([df_a, df_b], axis=1)
    else:
        # create a list for x
        x = []
        # load input data
    ...
    # load output data
    for name in filenames:
        data = load_file(prefix + 'Processed_Label_Data/' + group + name + '_label.csv')
        y.extend(data)
    df_y = pd.DataFrame(y)
    return df_x, df_y
...
def load_dataset(data_type, prefix, train_files, test_files, dimension):
    # load all train
...
def main():
    ...
    # data_type can be 'Processed_PatientSpace' or 'Processed_RawXY' or 'combine'
